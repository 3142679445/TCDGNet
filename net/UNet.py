...
# from .EFHPM import HFP
# from net.GeneralizationEnhancementFramework.Frequency_Framework_Module import FreqGeneralizationModule
...
#         self.conv_block_1 = DoubleConvBlock(3, 32)
...
#         # 2. 嵌入频域泛化框架（核心改造）
#         # 输入通道数=conv_block_out_5的通道数（512）
#         self.freq_module = FreqGeneralizationModule(
#             in_channels=512,
#             num_freq_bands=4,
#             use_adaptive_gate=True,
#             use_contrastive=True,
#             use_freq_consistency=True
#         )
...
# #         conv_block_out_5 = self.hfp(conv_block_out_5)
#         # ========== 频域框架增强（核心） ==========
#         # 由于UNet是单分支，x_cross和x_vssm均使用conv_block_out_5
#         if self.training:
#             # 训练阶段：返回增强特征 + 辅助损失
#             conv_block_out_5_enhanced, aux_losses = self.freq_module(
#                 x_cross=conv_block_out_5,
#                 x_vssm=conv_block_out_5,
#                 labels=labels
#             )
#         else:
#             # 推理阶段：仅返回增强特征，损失为空
...
#         # ========== 返回结果 ==========
#         if self.training:
#             return out, aux_losses  # 训练时返回分割结果+辅助损失
#         else:
#             return out  # 推理时仅返回分割结果
import torch
import torch.nn.functional as F
import torch.nn as nn
